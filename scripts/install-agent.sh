...
if [ ! -f "$CONFIG_DIR/.env" ]; then
    echo ""
    read -rp "Router URL [$DEFAULT_ROUTER_URL]: " router_url
    router_url="${router_url:-$DEFAULT_ROUTER_URL}"

    read -rp "Working directory [$HOME]: " working_dir
...
echo ""
echo "First run: verify your agent with Slack."
echo "Press Ctrl+C after verification to return here."
echo ""
read -rp "Press Enter to start the agent for verification..."

cd "$INSTALL_DIR"
set -a; source "$CONFIG_DIR/.env"; set +a
PYTHONPATH="$INSTALL_DIR" .venv/bin/python -m agent.main || true
